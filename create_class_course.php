...
// Fetch class records
$dataSql = "SELECT c.id, c.class_name, c.semester, c.is_active, 
    ...
    <tbody>
        <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
            <tr>
                ...
                <td>
                    <a href="edit_class.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-info" title="Edit Class">
                        <i class="fas fa-edit"></i>
                    </a>
                    <a href="delete_class.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this class?')" title="Delete Class">
                        <i class="fas fa-trash"></i>
                    </a>
                    <a href="toggle_class.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning" title="Toggle Active/Inactive">
                        <i class="fas fa-toggle-on"></i>
                    </a>
                    <a href="assign_courses.php?class_id=<?= $row['id'] ?>" class="btn btn-sm btn-success" title="Assign Courses">
                        <i class="fas fa-book"></i>
                    </a>
                    <a href="view_class_courses.php?class_id=<?= $row['id'] ?>" class="btn btn-sm btn-secondary" title="View Assigned Courses">
